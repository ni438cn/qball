...
////////////////////////////////////////////////////////////////////////////////
void ComplexMatrix::sum_columns_square_occ(std::vector<double>& occ_result, std::vector<double> const & occ_current) const //CS
{
  // Copy blocks of <blocksize> columns and print them on process (0,0)
  if ( m_ == 0 || n_ == 0 ) return;
  Context ctxtl(1,1);
  ...
  for ( int jb = 0; jb < nblocks; jb++ )
  {
    ...
    if ( t.active() )
    {
      ...
      for ( int jj = 0; jj < blocksize; jj++ )
      {
        for ( int ii = 0; ii < m_; ii++ )
        {
          occ_result[ii]+=norm(t.val[ii+t.mloc()*jj])*occ_current[jj+jb*maxbs]; //CS
